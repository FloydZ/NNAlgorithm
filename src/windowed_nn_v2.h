...
class WindowedNearestNeighbor2 : public NearestNeighbor {
  ...
  constexpr void get_window(uint64_t *k_lower, uint64_t *k_higher,
                            const uint64_t current_r) {
...
public:
  WindowedNearestNeighbor2(NNList &L1, NNList &L2, const uint64_t w,
                           const uint64_t r, const uint64_t N, const uint64_t d,
                           const bool find_all = true,
                           const uint64_t THRESHHOLD = 500,
                           const uint64_t epsilon = 0)
      : d_(double(d) / double(n)), k(n / r), dk(d_ * double(n) / double(r)),
        epsilon(epsilon),
  ...
  void windowed_SortList(NNList &L, const NNContainer &z,
    ...
    madvise(L1.data(), L1.size() * NNContainer::get_size() / 8,
            POSIX_MADV_WILLNEED | POSIX_MADV_SEQUENTIAL);
    madvise(L2.data(), L1.size() * NNContainer::get_size() / 8,
            POSIX_MADV_WILLNEED | POSIX_MADV_SEQUENTIAL);
#endif
  ...
  int NN() {
    ...
    for (int i = 0; i < N; ++i) {
      abort = NN_internal(0, L1.size(), 0, L2.size(), 0, k_lower, k_upper,
                          nullptr, nullptr);
      if (abort)
        return abort;
    }
  ...
  int NN(const NNContainer &gold1, const NNContainer &gold2) {
    ...
    while (not_found) {
      NN_internal(0, L1.size(), 0, L2.size(), 0, k_lower, k_upper, &gold1,
                  &gold2);
  ...
  int NN(const uint64_t nr_solutions) {
    ...
    while (true) {
      NN_internal(0, L1.size(), 0, L2.size(), 0, k_lower, k_upper, nullptr,
                  nullptr);
      ASSERT(sols_1.size() == sols_2.size());
...
private:
  ...
  int NN_internal(const uint64_t start1, const uint64_t end1,
                  ...
                  uint64_t current_r, const uint64_t k_lower,
                  const uint64_t k_upper, const NNContainer *gold1,
                  const NNContainer *gold2) {
    ...
    windowed_SortList(L1, z, k_lower, k_upper, start1, end1, new_start1,
                      new_end1);
    windowed_SortList(L2, z, k_lower, k_upper, start2, end2, new_start2,
                      new_end2);
    ...
    if (((new_end1 - new_start1) < THRESHHOLD) ||
      ...
      return quadratic_NN(new_start1, new_end1, new_start2, new_end2, gold1,
                          gold2);
    }
    ...
    for (uint64_t i = 0; i < N; ++i) {
      abort =
          NN_internal(new_start1, new_end1, new_start2, new_end2, new_current_r,
                      new_k_lower, new_k_upper, gold1, gold2);
      if (abort > 0) {
